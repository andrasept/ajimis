...
<link href="{{asset('css/plugins/sweetalert/sweetalert.css')}}" rel="stylesheet">
    ...
    <div class="ibox " >
        <div class="ibox-title">
            <h4>Edit Planning Refreshment</h4>
        </div>
        <div class="ibox-content" >
            ...
            @endif
          <div >
            <form action="{{route('delivery.planning_refreshment.update')}}" enctype="multipart/form-data" method="post">
                {{csrf_field()}}
                {{method_field("PUT")}}
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="training" id="training" value="{{$data->training}}" required>
                            @error('training') 
                        ...
                        <div class="form-group">
                            ...
                            <select name="user_id" id="user_id" style="width:100%" class=" form-control" required>
                                <option value="-">-</option>
                                @foreach ($mp as $item)
                                <option value="{{$item->npk}} " {{ $data->user_id == $item->npk ? "selected" : "" }}>{{$item->name}}</option>
                                @endforeach
                            </select>
                            @error('user_id') 
                                <div class="text-danger">
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control" name="plan_date_time" id="" value="{{$data->plan_date_time ?? date('Y-m-d')}}" required>
